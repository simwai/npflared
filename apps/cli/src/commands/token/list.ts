...
async function listTokensInteractively(local: boolean) {
	intro(chalk.bold(`babadeluxe-registry  token list  ${chalk.gray(local ? "local" : "remote")}`));
...
async function listTokensForPackage(argumentsValue: ListTokensArgs) {
	if (!argumentsValue.skipIntro) {
		intro(chalk.bold(`babadeluxe-registry  token list  ${chalk.gray(argumentsValue.local ? "local" : "remote")}`));
	}
