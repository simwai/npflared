...
This guide is for developers with little to no experience with Wrangler or Cloudflare Workers who are troubleshooting `403 Forbidden` errors in `babadeluxe-registry`.

## 1. Quick Start: Are Migrations Applied?
If you've recently updated `babadeluxe-registry` or added tables, the most common reason for a `403 Forbidden` is a missing database migration in production.
...
**Check these things:**
- **Glob Patterns:** Stars (`*`) in your token scope do NOT match slashes (`/`) by default in older versions of `babadeluxe-registry`. Ensure you're using the latest code which has the `{ dot: true, bash: true }` fix.
- **Example Scopes:**
...
- `babadeluxe-registry` handles this by decoding the name before checking permissions.
